...
def extrusion_calculator(setpoint_speed, runout_length, ratio, die_cavities, billet_temp_C=470, alloy='6xxx', mass_kg_m=1):
    ...
    base_force = 500 + 1200 * ((die_cavities - 1) / 3)
...
def calculate():
    ...
    billet_temp_C = float(data.get('billet_temp') or 0)
    alloy = data.get('alloy') or '6xxx'
    mass_kg_m = float(data.get('mass_kg_m') or 1)
    burp, puller_pct, ramp, force = extrusion_calculator(setpoint_speed, runout_length, ratio, die_cavities, billet_temp_C, alloy, mass_kg_m)
    return jsonify({'burp_psi': burp, 'puller_speed_pct': round(puller_pct), 'torque_ramp_time': round(ramp, 1), 'puller_force': force})
